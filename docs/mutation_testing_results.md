...
| 8   | `src/envelope/exception.filter.ts` | 298:11   | ConditionalExpression | `context.correlationId !== undefined ? { correlationId } : {}` -> `true ? ...` | The always-spread form injects `correlationId: undefined` into the builder input, but `buildErrorEnvelope` re-guards `input.correlationId !== undefined` and omits an undefined value, so the emitted envelope is byte-for-byte identical whether or not a correlation id is present.   |
